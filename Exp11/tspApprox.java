...
public class tspApprox {
    ...
    public static void main(String[] args) {
        List<List<Integer>> mst = primMST();
        List<Integer> tour = new ArrayList<>();
        boolean[] visited = new boolean[V];
        dfs(0, visited, mst, tour);
        tour.add(0); // return to starting point
    ...
    static void dfs(int u, boolean[] visited, List<List<Integer>> mst, List<Integer> tour) {
        visited[u] = true;
        tour.add(u);
        for (int v : mst.get(u)) {
            if (!visited[v])
    ...
    static List<List<Integer>> primMST() {
